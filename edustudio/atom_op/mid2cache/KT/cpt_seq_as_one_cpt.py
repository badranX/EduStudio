...
class M2C_CptSeqAsOneCpt(BaseMid2Cache):
    default_cfg = {}
    ...
    def cpt_as_exer_4undivided(df_exer, df):
        ...
        #tmp_mask = df_exer["cpt_seq:token_seq"].astype(bool)

        lbe = LabelEncoder()
        df_exer["cpt_seq:token_seq"] = lbe.fit_transform(df_exer["cpt_seq:token_seq"] ).tolist()
        df_exer["cpt_seq:token_seq"] = df_exer["cpt_seq:token_seq"].apply(
            lambda x: [x])
        #df.rename(columns={"cpt_seq:token_seq": "tmp"}, inplace=True)

        #df.rename(columns={'exer_id:token': 'exer_id_old:token'}, inplace=True)
        #df.drop(labels=["cpt_seq:token_seq"], axis=1, inplace=True)
        df = df.merge(df_exer[["exer_id:token", "cpt_seq:token_seq"]], on='exer_id:token', how='left').reset_index(drop=True)
        #df.drop(labels=['tmp'], axis=1, inplace=True)
        return df_exer, df
    ...
    def cpt_as_exer_4divided(df_exer, df_train, df_valid, df_test):
        ...
        #df_exer[df_exer["cpt_seq:token_seq"].astype(bool)] = []
        tmp = df_exer[~df_exer["cpt_seq:token_seq"].astype(bool)]
        lbe = LabelEncoder()
        tmp = lbe.fit_transform(tmp).tolist()
        df_exer['cpt_seq:token_seq'] = df_exer['cpt_seq:token_seq'].apply(
            lambda x: [x])
        ...
        df_train = df_train.merge(df_exer[["exer_id:token", "cpt_seq:token_seq"]], on='exer_id:token', how='left').reset_index(drop=True)
        ...
        if df_valid is not None:
            ...
            df_valid = df_valid.merge(df_exer, on='exer_id:token', how='left').reset_index(drop=True)
        ...
        df_test = df_test.merge(df_exer[["exer_id:token", "cpt_seq:token_seq"]], on='exer_id:token', how='left').reset_index(drop=True)
